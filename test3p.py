...
st.subheader('время упреждающего сигнала около 60 сек')
...
path = st.file_uploader("Choose a EXCEL or CSV file", type=["xlsx"])
...
if path:
    ...
    values = st.slider("id интервал", 0, maks, (0, maks))
    st.write(values)
    start_id = values[0]
    finish_id = values[1]

    if st.button('Начать анализ в указанных параметрах'):
        st.write("Начинаем анализ файла... {}".format(path.name))
        ...
        # сделаем предикт Stuckpipe 60_6:
        ...
        for i in range(len(predict_list) - 2):
        ...
        data2 = data[start_id:finish_id]
        print(data["predict_class"].value_counts())

        n = finish_id
        X_valid = X[finish_id]
        feat_list = ['BPOS', 'HKLD', 'STOR', 'ECD', "FLWI", "RPM", "SPPA"]
        Time = data["Date/Time"][finish_id]
        hole_number = data["hole"][finish_id]
        #hole_number = os.path.split('.')[0]

        # id_index = data.index[finish_id]
        # Stuckpipe = data["StuckPipe"][finish_id]
        ...
        for hole in data.hole.unique():
            plt_time_shift(data2, 'Date/Time', feat_list, hole)

        for i in range(start_id, finish_id+1):
            X_valid = X [i]
            X_n = X_norm[i]
            ...
            y_predict = data["predict_class"][i]
            # y_predict_proba = ctboost_model.predict_proba(X_n)  # вариант с вероятностью
            if y_predict==1:
            # if y_predict_proba[1] >= 0.7:
                os.system('say "Опасность прихвата 1 минуты"')
                st.write("ID: {}".format(id_index))
                st.header("Внимание! Высокая вероятность прихвата!")
