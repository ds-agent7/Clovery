...
st.write("(Анализ последней ситуации, графики, построчный анализ данных, ,без постпроцессинга)")
...
#model = "ctboost_predict-60-60.pkl"
...
for path in files:
    ...
    maks = data.index.max()
    start_id = data.index.min()
    finish_id = data.index.max()
    ...
    if button:
        ...
        #start_id = data1.index.min()
        #finish_id = data1.index.max()
        ...
        for i in range (start_id, finish_id+1):
            ...
            y_predict_proba_1 = data['predict_1'][i]  # вариант с вероятностью
            y_predict_1 = data["predict_class"][i]
