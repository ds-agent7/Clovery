...
# Обработка файла моделью
        model = pickle.load(open('finalized_model21.pkl', 'rb'))   #('finalized_model7.pkl', 
        pred = model.predict_proba(df)[:, 1]
        pred2 = pd.Series(pred).apply(lambda n: 1 if n>0.5 else 0) #0.572 578 для 17-норм 575-по 3 много 5.8 - сам прихват на 5 единиц
        #st.write('Разметка')
# Постпроцессинг
...
#                predict_list[i+2]=0
       ...
       # st.write('Разметка прихватов успешно завершена')
        ...
        for i in range(len(list_sheet)):
            if list_1[i]>5: 
                st.write(list_1[i], df_res['Date/Time'].loc[list_sheet[i]])
        ...
        if selected_chek:
           with pd.ExcelWriter(path_res, engine='xlsxwriter') as wb:    
               ...
               for i in list_sheet:
                ...
                chart.add_series({
                'categories': ['Sheet1', i-10, 23, i+20, 23],   
                'values':     ['Sheet1', i-10, 5, i+20, 5], 
                'name': 'BPOS',
                'y2_axis': True,
                'line':       {'color': 'red'}})
